...
struct RecipeDetailsView: View {
    @Binding var recipeModel: RecipeModel
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 10) {
                AsyncImage(url: URL(string: recipeModel.recipe.image)) // you need to implement this to load remote image
                    .aspectRatio(contentMode: .fit)
                
                
                HStack {
                    VStack {
                        Text(recipeModel.recipe.label)
                            .font(.title)
                            .fontWeight(.bold)
                        
                        Text("Source: \(recipeModel.recipe.source)")
                            .font(.subheadline)
                    }
                    Spacer()
                    ...
                    Button(action: {
                        ...
                        if isLiked {
                            favouriteRecipesModel.like(recipe: recipeModel)
                        } else {
                            favouriteRecipesModel.dislike(recipe: recipeModel)
                        }
                    },
                           label: {
                        if isLiked {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    LazyHStack(spacing: 10) {
                        ForEach(recipeModel.recipe.healthLabels, id: \.self) { label in
                            Text(label)
                                .padding(.vertical, 5)
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 10) {
                        ForEach(recipeModel.recipe.dietLabels, id: \.self) { label in
                            Text(label)
                                .padding(.vertical, 5)
                                .padding(.horizontal, 10)
                ...
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    ForEach(recipeModel.recipe.ingredients, id: \.text) { ingredient in
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    LazyVGrid(columns: [GridItem(), GridItem()], spacing: 20) {
                        NutrientRow(label: "Calories", value: "\(recipeModel.recipe.calories ?? 0)", color: Color.purple)
                        NutrientRow(label: "Total Weight", value: "\(recipeModel.recipe.totalWeight ?? 0)", color: Color.orange)
                        NutrientRow(label: "Total Time", value: "\(recipeModel.recipe.totalTime ?? 0)", color: Color.blue)
                        NutrientRow(label: "Energy", value: "\(recipeModel.recipe.totalNutrients.energy.quantity) \(recipeModel.recipe.totalNutrients.energy.unit)", color: Color.green)
                        NutrientRow(label: "Fat", value: "\(recipeModel.recipe.totalNutrients.fat.quantity) \(recipeModel.recipe.totalNutrients.fat.unit)", color: Color.red)
                        NutrientRow(label: "Carbs", value: "\(recipeModel.recipe.totalNutrients.carbs.quantity) \(recipeModel.recipe.totalNutrients.carbs.unit)", color: Color.yellow)
                        NutrientRow(label: "Protein", value: "\(recipeModel.recipe.totalNutrients.protein.quantity) \(recipeModel.recipe.totalNutrients.protein.unit)", color: Color.blue)
                    }
                }
                ...
                HStack {
                    Spacer()
                    ShareLink(item: URL(string: recipeModel.recipe.url)!){
        ...
        .navigationTitle(recipeModel.recipe.label)
        .onAppear {
            for favouriteRecipeModel in favouriteRecipesModel.recipes {
                if favouriteRecipeModel.id == recipeModel.id {
                    recipeModel.isLiked = true
                    break
                } else {
                    recipeModel.isLiked = false
                }
            }
            recentRecipesModel.load(recipe: recipeModel)
        }
    }
}
    ...
    static var previews: some View {
        let recipe = Recipe(uri: "http://www.edamam.com/ontologies/edamam.owl#recipe_16f636c6ef7f82621fa9bc1ab08c1c96",
                            ...
                            totalDaily: AllNutrients(
                                ...
                                carbs: Nutrient(label: "carbs", quantity: 172.82856749999996, unit: "g"),
                                protein: Nutrient(label: "protein", quantity: 16.09692, unit: "g")))
        RecipeDetailsView(recipeModel: .constant(RecipeModel(recipe: recipe, isLiked: false)), isLiked: false)
